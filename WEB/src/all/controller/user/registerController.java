...
public class registerController {
	...
	public String register(ModelMap model, @ModelAttribute("user") User user, @RequestParam("email")String to, 
		...
		if(user.getUsername().trim().length() < 6) {
			errors.rejectValue("username", "user", "Tài khoản quá ngắn");
		}
		else if (user.getUsername().trim().length() > 50) {
			errors.rejectValue("username", "user", "Tài khoản quá dài");
		}
		
		if(user.getPassword().trim().length() == 0) {
			errors.rejectValue("password", "user", "Vui lòng điền mật khẩu");
		}
		else if (user.getPassword().trim().length() < 6) {
			errors.rejectValue("password", "user", "Mật khẩu quá ngắn");
		}
		
		if(!password.equals(password1)) {
			errors.rejectValue("password", "user", "Mật khẩu không trùng khớp");
		}
		
		String regex = "^[\\w-_\\.+]*[\\w-_\\.]\\@([\\w]+\\.)+[\\w]+[\\w]$";
		if(user.getEmail().trim().length() == 0) {
			errors.rejectValue("email", "user", "Vui lòng điền email");
		}
		...
		if(checkbox == false) {
			model.addAttribute("checkbox", "Đồng ý các điều khoản");
		}
		...
		if(errors.hasErrors()) {
			...
			try {
				...
				String body = "Thank you for beliving in my service";
